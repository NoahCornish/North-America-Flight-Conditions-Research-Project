...
MISSING_FC <- file.path(DATA_DIR, "airports_missing_flight_category.csv")
...
col_loc  <- norm_name(raw_df, c("observed_local","local_time","local"), "observed_local")
...
# -------- Recompute observed_local from IANA tz --------
...
if (nrow(missing_fc) > 0) {
  if (file.exists(MISSING_FC)) {
    old <- suppressMessages(read_csv(MISSING_FC, show_col_types = FALSE))

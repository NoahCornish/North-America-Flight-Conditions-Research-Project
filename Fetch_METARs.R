...
#
# IMPORTANT: Keep the exact same column set/order as before.
# We *replace* observed_local with true station-local wall time (string),
# computed via IANA time zones per station.
# No new columns are added to the CSV.
# -------------------------------------------------------------------
...
normalize_row <- function(r){
  ...
  tibble(
    ...
    observed_local  = if(!is.na(obs_utc)) with_tz(obs_utc,"UTC") else NA, # placeholder; replaced later
...
# Step 1: fetch ALL airports in CA/US (grab iata_code for tz join)
...
# From Matt Johnson-Pint’s gist (timezones.csv)
...
norm_tz_cols <- function(df) {
  ...
  for (nm in names(rename_map)) {
    src <- rename_map[[nm]]
    if (!is.null(src) && !is.na(src) && src != nm) names(df)[names(df)==src] <- nm
  }
  df
}
...
if (!is.null(res) && nrow(res) > 0) {
  ...
  # Compute per-row local time using IANA tz.
  # Store as ISO string in existing observed_local column (no new columns).
  ...
  # Drop the helper tz columns before writing (to keep schema identical)
  res <- res %>% select(
